...
#             print(x.device, y.device)
            ...
            log_mu_x = torch.logsumexp(lmbda_dual*dist_mat, dim=1) - np.log(dist_mat.shape[0])
...
def calc_RD_curve(lmbdas, loader, N=1000, data_name="MNIST"):
    ...
    for lam in lmbdas:
        model = GenRD(lmbda=lam)
        checkpoint = torch.load(f'trained_lagr/trained_{data_name}/NERD_{data_name}_lmbda{lam:.3f}.pt')
...
if __name__ == '__main__':
    ...
    if args.data_name == "MNIST":
        dm = dataloaders.MNISTDataModule(5000)
